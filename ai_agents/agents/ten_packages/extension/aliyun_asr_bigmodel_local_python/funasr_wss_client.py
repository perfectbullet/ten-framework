...
parser.add_argument(
    "--host", type=str, default="192.168.8.233", required=False, help="host ip, localhost, 0.0.0.0"
)
...
parser.add_argument("--send_without_sleep",action="store_true",default=True, help="if audio_in is set, send_without_sleep")
...
async def message(id):
    ...
    text_print_final = ""  # 最终确定的文本（2pass-offline 或 offline）
    if args.output_dir is not None:
        ibest_writer = open(
    ...
    try:
        while True:
            ...
            # 服务端返回的是全量累积文本，直接使用（不再累加）
            if mode == "2pass-online":
                # 流式中间结果，清屏打印
                # os.system("clear")
                print(f"\r{id}: [流式] {text}")
            elif mode == "2pass-offline":
                # 最终结果，更新并打印
                # os.system("clear")
                print(f"\r{id}: [最终] {text}")
            elif mode == "online":
                # os.system("clear")
                print(f"\r{id}: [online] {text}")
            elif mode == "offline":
                text_print_final = text
                print(f"\r{id}: [offline] {wav_name}: {text_print_final}")

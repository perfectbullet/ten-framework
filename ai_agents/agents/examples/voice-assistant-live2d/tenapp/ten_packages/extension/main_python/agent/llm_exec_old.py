...
from .langgraph_agentic_rag_new import graph
...
class LLMExec:
    ...
    async def _retrieve_relevant_docs(self, query: str) -> list[str]:
        ...
        try:
            ...
            self.ten_env.log_info(f"_retrieve_relevant_docs inputs: {inputs}")
            output = graph.invoke(inputs)
            self.ten_env.log_info(f"_retrieve_relevant_docs output: {output}")
            last_msg = output["messages"][-1]
            docs = []
            if isinstance(last_msg, ToolMessage):
                docs = [last_msg.content]
            self.ten_env.log_info(f"_retrieve_relevant_docs: {docs}")
            return docs
        except Exception as e:
